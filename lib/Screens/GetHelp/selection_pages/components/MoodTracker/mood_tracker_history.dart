...
class _MoodTrackerHistoryState extends State<MoodTrackerHistory> {
  ...
  Future<List<MoodTracker>> generateMoodTrackerList() async{
    ...
    var response2 = await http.post(Uri.parse(data));
    var decodedMoodTracker = json.decode(response2.body).cast<Map<String, dynamic>>();
  ...
  void buildDataGridRow(){
    dataGridRows = moodTrackerList.map<DataGridRow>((dataGridRow){
      return DataGridRow(cells: [
        DataGridCell(columnName: 'Date and Time', value: dataGridRow.date),
        DataGridCell(columnName: 'Mood', value: dataGridRow.date),
        DataGridCell(columnName: 'Comment', value: dataGridRow.date),
      ]);
